...
import {
  ...
  MotoCountryKey,
  MotoEntry,
} from '@/data/catalogMoto';
import { buildMotoCollectionContent } from '@/lib/motoCollectionContent';
...
const countryNamesMap: Record<MotoCountryKey, string> = {
  japan: 'Япония',
  europe: 'Европа',
  usa: 'США',
};
...
const MotoCollection = ({ mode }: { mode: 'country' | 'brand' }) => {
  const { country, brand } = useParams();

  let data: CollectionData | null = null;

  if (mode === 'country' && country && country in countryNamesMap) {
    const key = country as MotoCountryKey;
    const entries = motoEntriesByCountry(key);
    const gen = countryGenitive[key] ?? countryNamesMap[key];
    data = {
      title: `Мотоциклы из ${gen} на заказ — ${models(entries.length)} и цены | Регион Логистик`,
      h1: `Мотоциклы из ${gen} на заказ`,
      description: `${models(entries.length)} мотоциклов из ${gen} под заказ: характеристики, ориентировочные цены под ключ, подбор и доставка в Россию компанией Регион Логистик.`,
      keywords: [
        `мотоцикл из ${gen}`,
        `мотоциклы из ${gen}`,
        `купить мотоцикл из ${gen}`,
        `заказать мотоцикл из ${gen}`,
        'Регион Логистик',
      ].join(', '),
      url: `https://rlogistik.ru/moto/${key}`,
      entries,
      intro: `Подбираем и привозим мотоциклы из ${gen} под ключ: проверка, выкуп, доставка, растаможка и постановка на учёт. В каталоге ${models(entries.length)} с ориентировочными ценами.`,
    };
  }

  if (mode === 'brand' && brand) {
    const found = findMotoBrand(brand);
    if (found) {
      data = {
        ...
        description: `${models(found.entries.length)} мотоциклов ${found.brand} под заказ: характеристики, ориентировочные цены под ключ, подбор и доставка в Россию компанией Регион Логистик.`,
        keywords: [
          found.brand,
        ...
        url: `https://rlogistik.ru/moto/brand/${found.slug}`,
        entries: found.entries,
        intro: `Привозим мотоциклы ${found.brand} под ключ из Японии, Европы и США. В каталоге ${models(found.entries.length)} с характеристиками и ориентировочной ценой.`,
      };
    }
  }
  ...
  const extra = buildMotoCollectionContent(
    data.entries,
    mode,
    mode === 'country' ? (country as string) : findMotoBrand(brand as string)!.brand
  );
    ...
    <div className="min-h-screen bg-white">
      ...
      <main className="container py-16">
        <nav className="text-sm text-muted-foreground mb-4">
          ...
          <Link to="/moto" className="hover:text-primary">Мотоциклы</Link>
          <span className="mx-1.5">/</span>
        ...
        {extra.stats.minTotal > 0 && (
          <section className="mt-10 max-w-3xl">
            ...
            <div className="rounded-2xl border border-border overflow-hidden">
              ...
              <div className="flex items-center justify-between gap-4 px-4 py-3 text-sm border-t border-border">
                <span className="text-muted-foreground">Цена мотоцикла без доставки, от</span>
        ...
        <div className="mt-10 text-sm text-muted-foreground max-w-3xl space-y-2">
          <p>
            Минимальная цена мотоцикла в подборке — {minPriceEntry.variant.price} ({minPriceEntry.variant.model}), без доставки и растаможки.
            Всего: {models(data.entries.length)}. Цены ориентировочные, точную стоимость под ключ
            рассчитает менеджер.
          </p>
          <p>
            <Link to="/moto" className="text-primary hover:underline">
              Весь каталог мотоциклов
            </Link>{' '}
            — {models(motoEntries.length)} из трёх стран.
          </p>
        </div>
